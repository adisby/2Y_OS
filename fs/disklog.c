...
PUBLIC void dump_fd_graph(const char * fmt, ...)
{
	...
	for (i = 0; i < SECTOR_SIZE / sizeof(struct inode); i++,p+=INODE_SIZE) {
		...
		if (pinode->i_zone[0] == 0)
			continue;
		int start_sect;
		int end_sect;
		if (pinode->i_mode) {
			if (pinode->i_zone[0] < sb->n_1st_sect) {
				panic("should not happen: %x < %x.",
				      pinode->i_zone[0],
				      sb->n_1st_sect);
			}
			start_sect =  pinode->i_zone[0] - sb->n_1st_sect + 1;
			...
			start_sect = MAJOR(pinode->i_zone[0]);
			end_sect = MINOR(pinode->i_zone[0]);
			logbufpos += sprintf(logbuf + logbufpos, "\t\t\"inodearray%d\" [\n", i+1);
			logbufpos += sprintf(logbuf + logbufpos, "\t\t\tlabel = \"<f0> %d"
					     "|<f2> i_size:0x%x"
					     "|<f3> dev nr: (%xh,%xh)",
	...
	logbufpos += sprintf(logbuf + logbufpos, "\n\t\tcolor=lightgrey;\n");
	sb = get_super_block(root_inode->i_dev);
	int dir_blk0_nr = root_inode->i_zone[0];
	int nr_dir_blks = (root_inode->i_size + SECTOR_SIZE - 1) / SECTOR_SIZE;
	int nr_dir_entries =
	  root_inode->i_size / DIRENT_SIZE; /**
					       * including unused slots
					       * (the file has been deleted
					       * but the slot is still there)
					       */
	...
	for (i = 0; i < nr_dir_blks; i++) {
		...
		memcpy(_buf, logdiskbuf, SECTOR_SIZE);
		pde = (struct dirent *)_buf;
		for (j = 0; j < SECTOR_SIZE / DIRENT_SIZE; j++,pde++) {
			if (pde->d_ino) {
				memcpy(filename, pde->d_name, MAX_FILENAME_LEN);
				if (filename[0] == '.')
					filename[0] = '/';
