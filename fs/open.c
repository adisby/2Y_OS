...
PRIVATE struct inode * new_inode(int dev, int d_ino, int start_sect);
PRIVATE void new_dirent(struct inode * dir_inode, int d_ino, char * filename);
...
PUBLIC int do_open()
{
	...
	if (d_ino == INVALID_INODE) { /* file not exists */
		if (flags & O_CREAT) {
...
do_mkdir ()
{
	...
	printl("{FS} Start to mkdir\n");
	struct inode *dir_inode = p_proc_ready->pwd;

	int d_ino = alloc_imap_bit(dir_inode->i_dev);
	int free_sect_nr = alloc_smap_bit(dir_inode->i_dev,
					  NR_DIRECT_ZONE);

	printl("{FS} imap and smap allocated\n");
	struct inode * new_inode = get_inode(dir_inode->i_dev, d_ino);

	new_inode->i_mode = I_DIRECTORY;

	new_inode->i_size = 0;
	for (i = 0; i < NR_DIRECT_ZONE; i++) {
		new_inode->i_zone[i] = free_sect_nr + i;
	}
	new_inode->i_nr_sects = NR_DIRECT_ZONE;

	new_inode->i_dev = dir_inode->i_dev;
	new_inode->i_cnt = 1;
	new_inode->i_num = d_ino;

	/* write to the inode array */
	sync_inode(new_inode);

	printl("{FS} inode sync done\n");
	...
	/* add the dirent in the parent directory */
	new_dirent(dir_inode, new_inode->i_num, filename);

	/* add the "." and ".." dirent in new directory */
	new_dirent(new_inode, new_inode->i_num, ".");
	new_dirent(new_inode, dir_inode->i_num, "..");

	//release the inode
	put_inode(new_inode);
	printl("{FS} dirent allocated\n");

	return 0;
}		/* -----  end of function do_mkdir  ----- */
/*****************************************************************************
 *                                create_file
 *****************************************************************************/
/**
...
PRIVATE struct inode * create_file(char * path, int flags)
{
	...
	int free_sect_nr = alloc_smap_bit(dir_inode->i_dev,
					  NR_DIRECT_ZONE);
	struct inode *newino = new_inode(dir_inode->i_dev, d_ino,
					 free_sect_nr);

	new_dirent(dir_inode, newino->i_num, filename);
...
{
	int i;
	struct inode * new_inode = get_inode(dev, d_ino);

	new_inode->i_mode = I_REGULAR;
	new_inode->i_size = NR_DIRECT_ZONE * SECTOR_SIZE;
	for (i = 0; i < NR_DIRECT_ZONE; i++) {
		new_inode->i_zone[i] = start_sect + i;
	}
	new_inode->i_nr_sects = NR_DIRECT_ZONE;
 ...
 * @param d_ino   I-node nr of the new file.
...
PRIVATE void new_dirent(struct inode *dir_inode,int d_ino,char *filename)
{
	...
	for (i = 0; i < nr_dir_blks; i++) {
		...
		for (j = 0; j < SECTOR_SIZE / DIRENT_SIZE; j++,pde++) {
			...
			if (pde->d_ino == 0) { /* it's a free slot */
				new_de = pde;
				break;
			}
		}
	...
	new_de->d_ino = d_ino;
	strcpy(new_de->d_name, filename);

	/* write dir block -- ROOT dir block */
	WR_SECT(dir_inode->i_dev, dir_inode->i_zone[i]);
