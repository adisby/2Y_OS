...
PUBLIC int do_unlink()
{
	...
	if (pin->i_mode != I_REGULAR && pin->i_mode != I_DIRECTORY) { /* can only remove regular files and directorys */
	 ...
	 */
	bit_idx  = pin->i_zone[NO_ZONE] - sb->n_1st_sect + 1;
	byte_idx = bit_idx / 8;
	...
	memset(pin->i_zone,
			(zone_t)NO_ZONE,
			sizeof(zone_t) * (THIRD_LEVEL_ZONE  + 1));
	pin->i_nr_sects = 0;
	sync_inode(pin);
	...
	int nr_dir_entries =
		dir_inode->i_size / DIRENT_SIZE; /* including unused slots
						     * (the file has been
	...
	for (i = 0; i < nr_dir_blks; i++) {
		...
		for (j = 0; j < SECTOR_SIZE / DIRENT_SIZE; j++,pde++) {
			...
			if (pde->d_ino == inode_nr) {
				/* pde->inode_nr = 0; */
				memset(pde, 0, DIRENT_SIZE);
				WR_SECT(dir_inode->i_dev, dir_inode->i_zone[i]);
				flg = 1;
				break;
			}

			if (pde->d_ino != INVALID_INODE)
