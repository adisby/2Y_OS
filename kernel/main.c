...
void Init()
{
	/* init the processes working directory and root directory */
	int index;
	for (index = 0; index < NR_TASKS + NR_PROCS; index++ ) {
		proc_table[index].pwd = root_inode;
		proc_table[index].root = root_inode;
		root_inode->i_cnt += 2;
	}
